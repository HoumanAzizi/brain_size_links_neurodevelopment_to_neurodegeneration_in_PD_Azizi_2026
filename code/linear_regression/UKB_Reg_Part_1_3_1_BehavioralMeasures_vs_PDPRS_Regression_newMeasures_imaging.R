...
main_output_folder <- 'Outputs/PD_PRS_Regression_Results/with_all_pathwayPRS_Behavioral_imaging/'
...
for (model_sex in model_sex_options) {
  ...
  if (model_sex == 'all') {
    Confounds <- paste0('Age + Age2 + Sex + Age:Sex + Age2:Sex + Days + Days2 + Center_Number + Genotype_batch + ', PCAs)
  ...
  if (!file.exists('Data/Current_UKB/UKB_Behavioral/UKB_BehavioralMeasures_vs_PDPRS_Data_Nov2025.csv')) { # Sept25 does not include Sleep duration
    ############################# Reading Data #############################
    ...
    # UKB Behavioral Variables
    UKB_beh <- as.data.frame(read_delim('Data/Current_UKB/UKB_Behavioral/subset_2025_Sept4_Beh_newMeasures_wide.tsv', delim = "\t", col_names = TRUE, na = "", quote = "")) # previously subset_Nov24_PLS_wide.tsv
    # New PD-PRS
    #PRS <- read.csv('Data/Genetic/PD_PRS/PD_PRScs_Lang.csv') # run this if want Lang's version (ice)
    ...
    UKB_beh <- UKB_beh %>% select(-`Diagnoses - main ICD10_41202`, -`Non-cancer illness code, self-reported_20002`)
    
    
    
    ############################# Fix the UKB_beh data #############################
    UKB_beh <- UKB_beh %>% filter(InstanceID == 2)
    ## Fixing income
    ...
    ## Set column names
    UKB_beh <- UKB_beh %>% select(-starts_with('Volume'), -ArrayID, -InstanceID, -Height_12144)
    colnames(UKB_beh)
    UKB_beh <- UKB_beh %>% select(SubjectID, `Fluid intelligence score_20016`, `Coffee intake_1498`, `Body mass index (BMI)_21001`, `Age when attended assessment centre_21003`,
                                  `Index of Multiple Deprivation (England)_26410`, `Pack years of smoking_20161`, `Sleep duration_1160`, Constipation, Orthostatic_hypotension, Urinary_incontinence,
    ...
    UKB_data_final <- UKB_data_final %>% mutate(Center_Number = case_when(cn == "Cheadle (imaging)" ~ 1,
                                                                          ...
                                                                          cn == "Reading (imaging)" ~ 3, 
                                                                          cn == "Bristol (imaging)" ~ 4)) %>% select(-c(cn))
    ...
    ############################# Save to reuse later #############################
    write.csv(UKB_BL, 'Data/Current_UKB/UKB_Behavioral/UKB_BehavioralMeasures_vs_PDPRS_Data_Nov2025.csv', row.names = FALSE)
  }
  UKB_BL <- read.csv('Data/Current_UKB/UKB_Behavioral/UKB_BehavioralMeasures_vs_PDPRS_Data_Nov2025.csv')
  UKB_BL <- UKB_BL %>% filter(Sex %in% sex_to_include)
  PD_PRScs_list <- colnames(UKB_BL)[grep("^PD_PRScs_GP2", colnames(UKB_BL))]
  ...
  for (current_prs in PD_PRScs_list) {
    ...
    UKB_BL_model$Days <- scale(UKB_BL_model$Days)[,1]
    UKB_BL_model$Days2 <- scale(UKB_BL_model$Days2)[,1]
