...
GWAS_files <- c('/Volumes/WorkSSD-Houman/GWAS/Zhao2020_GWAS/FA_average_allTracts_ukb_phase1to3_dti441_dec21_2019_pheno7.fastGWA','/Volumes/WorkSSD-Houman/GWAS/Zhao2020_GWAS/MD_average_allTracts_ukb_phase1to3_dti441_dec21_2019_pheno259.fastGWA')
...
all_results$GWAS <- sapply(GWAS_files, function(x) {substr(basename(x), 1, 20)})

## Loop over each file
for (file in GWAS_files) {
  current_GWAS <- substr(basename(file), 1, 20)
  print(current_GWAS)
  ...
  tryCatch({
    ...
    if (!file.exists(paste0('Data/Exposure_MR_files_generated/', current_GWAS, '_Zhao_MR.csv'))) {
      ## Read exposure data
      ...
      write.table(exp_filtered, paste0('Data/Exposure_MR_files_generated/', current_GWAS, '_Zhao_MR.csv'), 
                  sep=',', row.names=FALSE) # Save to a csv for further processing
      rm(exp_filtered)
    }
    ## Read final exposure data
    exp_data <- read_exposure_data(paste0('Data/Exposure_MR_files_generated/', current_GWAS, '_Zhao_MR.csv'), 
                                   sep=",", 
...
write.csv(all_results, paste0('Outputs/result_summary_allMeasures/Zhao_GWAS_MR_Results_complete.csv'))
